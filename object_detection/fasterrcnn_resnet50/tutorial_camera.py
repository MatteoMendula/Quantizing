...
def get_compressed_image_from_numpy(numpy_image, quality=40):
    from io import BytesIO # "import StringIO" directly in python2
    from PIL import Image
    import os
...
def plot_results(detection_result, image, classes_to_labels):
...
img = get_compressed_image_from_numpy(frame, jpeg_compression)
...
my_prediction = {}
my_prediction["boxes"] = prediction[0]["boxes"].cpu().detach().numpy().tolist()
my_prediction["labels"] = prediction[0]["labels"].cpu().detach().numpy().tolist()
my_prediction["scores"] = prediction[0]["scores"].cpu().detach().numpy().tolist()

print("--- %s seconds ---" % (time.time() - start_time))
print(prediction)

classes_to_labels= get_coco_object_dictionary()
plot_results(my_prediction, img, classes_to_labels)
